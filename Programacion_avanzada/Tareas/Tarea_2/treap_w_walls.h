...
struct Treap
{
 ...
 left y right y lo igualamos a la raíz para tener de nuevo el árbol que queríamos*/
	...
	pair<Node,Node> SplitForWall(long long int p)
	{
		Node m;
		SplitByPos(root,p,root,m);
		return make_pair(root,m);
	}
};

struct BorderTreap
{
 //Vamos a intentar resolver el problema de los muros con un vector de treaps
 vector<Treap> WallTreap;
 map<long long int, long long int> posicion; //Posición en el que empieza el treap y en donde se encuentra dentro del vecto
 //vector de treaps
 typename vector<Treap>::iterator it;
 vector<long int> max_pos;
 BorderTreap()
 {
	 //Creamos un vector con un solo treap
	 Treap treap;
	 WallTreap.push_back(treap);
 }
 ~BorderTreap(){}
 void print()
 {
	 for(it=WallTreap.begin();it!=WallTreap.end();++it) (*it).print();
 }
 long long int Walls()
 {
	 return WallTreap.size();
 }
 /*void AddWall(long long int p)
 {
